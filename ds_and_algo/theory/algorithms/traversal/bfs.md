...
Breadth-First Search (BFS) is a `Graph traversal` algorithm that can also be used to traverse trees. When BFS is applied specifically to a tree, it is commonly called `Breadth-First Traversal (BFT)` or `Level-Order Traversal`.

For example, a [Binary Search Tree (BST)](../../data_structures//trees/binary_search_tree.md) data structure can have a [BFT method](../../../implementations/python/algorithms/traversal/bfs.py) that visits all nodes level by level. However, the standard [BST search() method](../../../implementations/python/data_structures/trees/binary_search_tree.py) is not a BFS, it uses the BST ordering property (`left < root < right`) to decide which subtree to search and gives `O(log n)` time complexity for a balanced BST.
...
- Characteristics of BFS:
  ...
  - Not ideal if the Graph or Tree is too wide.
...
- You need the shortest path in an unweighted graph (or when all edges have equal weight).
- You want to explore a graph level by level, where the order of nodes within a level depends on the graph's representation, such as the order of neighbors in the adjacency list.
- You want level-order traversal of a tree.
