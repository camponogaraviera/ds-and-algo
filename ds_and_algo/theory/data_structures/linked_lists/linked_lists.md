...
`Linked Lists` are not native (built-in) data structures in `Python` and `JavaScript`. However, they can be implemented with `classes`.
...
- Use a `Vector` if there are too many `pop operations (removal from the end)` and `look-up-by index`.
